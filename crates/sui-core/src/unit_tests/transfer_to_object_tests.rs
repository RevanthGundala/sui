...
// 6. Execute `tx2` and verify that it can be executed, but will result in a Move abort.
async fn verify_tto_not_locked(flipper: bool) {
    ...
    let (valid_effects, invalid_effects) = if flipper {
        let TransactionEffects::V1(valid_effects) = runner.execute_certificate(valid_cert).await;
        let TransactionEffects::V1(invalid_effects) =
            runner.execute_certificate(cert_for_fake_parent).await;
...
async fn test_tto_not_locked() {
    verify_tto_not_locked(true).await;
    verify_tto_not_locked(false).await;
}
